#include "aes.h"
#include <string.h>

/* ---------- Tables ---------- */

static const uint8 sbox[256] = {

};

static const uint8 rcon[11] =
{0x00,0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1B,0x36};

/* ---------- Helpers ---------- */

static uint8 xtime(uint8 x) {
    return (x << 1) ^ ((x & 0x80) ? 0x1b : 0);
}

static void add_round_key(uint8 *s, uint8 *rk) {
    for (int i = 0; i < 16; i++) s[i] ^= rk[i];
}

static void sub_bytes(uint8 *s) {
    for (int i = 0; i < 16; i++) s[i] = sbox[s[i]];
}

static void shift_rows(uint8 *s) {
    uint8 t;
    t=s[1]; s[1]=s[5]; s[5]=s[9]; s[9]=s[13]; s[13]=t;
    t=s[2]; s[2]=s[10]; s[10]=t; t=s[6]; s[6]=s[14]; s[14]=t;
    t=s[3]; s[3]=s[15]; s[15]=s[11]; s[11]=s[7]; s[7]=t;
}

static void mix_columns(uint8 *s) {
    for (int i=0;i<4;i++) {
        uint8 *c=s+4*i;
        uint8 t=c[0]^c[1]^c[2]^c[3];
        uint8 u=c[0];
        c[0]^=t^xtime(c[0]^c[1]);
        c[1]^=t^xtime(c[1]^c[2]);
        c[2]^=t^xtime(c[2]^c[3]);
        c[3]^=t^xtime(c[3]^u);
    }
}

/* ---------- Public API ---------- */

void aes_key_expansion(uint8 *key, uint8 *rk) {
    memcpy(rk, key, 16);
    uint8 temp[4];
    int bytes=16, r=1;

    while (bytes < 176) {
        memcpy(temp, rk+bytes-4, 4);
        if (bytes % 16 == 0) {
            uint8 t=temp[0];
            temp[0]=sbox[temp[1]]^rcon[r++];
            temp[1]=sbox[temp[2]];
            temp[2]=sbox[temp[3]];
            temp[3]=sbox[t];
        }
        for (int i=0;i<4;i++) {
            rk[bytes]=rk[bytes-16]^temp[i];
            bytes++;
        }
    }
}

void aes_encrypt_block(uint8 *in, uint8 *out, uint8 *rk) {
    uint8 s[16];
    memcpy(s,in,16);

    add_round_key(s,rk);
    for (int r=1;r<=9;r++) {
        sub_bytes(s); shift_rows(s); mix_columns(s);
        add_round_key(s,rk+16*r);
    }
    sub_bytes(s); shift_rows(s);
    add_round_key(s,rk+160);

    memcpy(out,s,16);
}
